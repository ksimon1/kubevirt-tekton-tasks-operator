...
import (
	...
	tekton "github.com/kubevirt/kubevirt-tekton-tasks-operator/api/v1alpha1"
...
func (r *tektonTasksReconciler) reconcileOperands(sspRequest *common.Request) ([]common.ReconcileResult, error) {
...
func isInitialized(ssp *tekton.TektonTasks) bool {
...
func updateStatus(request *common.Request, reconcileResults []common.ReconcileResult) error {
	...
	if len(notAvailable) == 0 && len(progressing) == 0 && len(degraded) == 0 {
		...
		tektonStatus.ObservedVersion = common.GetOperatorVersion()
	} else {
		tektonStatus.Phase = lifecycleapi.PhaseDeploying
	}
